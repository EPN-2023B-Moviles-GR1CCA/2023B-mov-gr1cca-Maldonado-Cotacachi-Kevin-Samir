...
import com.example.uber_conductor.Fragmentos.ModalBottomSheetTripInfo
...
import com.example.uber_conductor.Modelos.Precio
...
import com.example.uber_conductor.Providers.ConfigProvider
import com.example.uber_conductor.Providers.GeoProvider
import com.example.uber_conductor.Providers.HistorialProvider
import com.example.uber_conductor.Providers.NotificacionProvider
import com.example.uber_conductor.Providers.ReservaProvider
import com.example.uber_conductor.databinding.ActivityMapaViajeBinding
...
class MapaViaje : AppCompatActivity(), OnMapReadyCallback, Listener, DirectionUtil.DirectionCallBack,
    SensorEventListener {
    ...
    private var booking: Reserva? = null
    private var client: Cliente? = null
    private var markerOrigin: Marker? = null
    private var bookingListener: ListenerRegistration? = null
    private lateinit var binding: ActivityMapaViajeBinding
    ...
    private val historyProvider = HistorialProvider()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityMapaViajeBinding.inflate(layoutInflater)
            ...
            override fun onResponse(call: Call<FCMResponse>, response: Response<FCMResponse>) {
                if (response.body() != null) {
                    ...
                    if (response.body()!!.success == 1) {
                        Toast.makeText(this@MapaViaje, "Se envio la notificacion", Toast.LENGTH_LONG).show()
                    }
                    else {
                        Toast.makeText(this@MapaViaje, "No se pudo enviar la notificacion", Toast.LENGTH_LONG).show()
                    }

                }
                else {
                    Toast.makeText(this@MapaViaje, "hubo un error enviando la notificacion", Toast.LENGTH_LONG).show()
                }
            }
    ...
    private fun getBooking() {
        bookingProvider.getBooking().get().addOnSuccessListener { query ->
            if (query != null) {
                ...
                if (query.size() > 0) {
                    booking = query.documents[0].toObject(Reserva::class.java)
...
//        easyWayLocation?.startLocation();
    ...
    private fun getPrices(distance: Double, time: Double) {
        ...
        configProvider.getPrices().addOnSuccessListener { document ->
            if (document.exists()) {
                val prices = document.toObject(Precio::class.java) // DOCUMENTO CON LA INFORMACION
    ...
    private fun goToCalificationClient() {
        val i = Intent(this, CalificacionCliente::class.java)
