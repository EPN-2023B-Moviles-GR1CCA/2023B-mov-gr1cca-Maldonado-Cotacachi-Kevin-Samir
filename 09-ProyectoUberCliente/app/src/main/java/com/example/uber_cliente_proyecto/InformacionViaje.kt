...
import com.example.uber_cliente_proyecto.Modelos.Precio
...
import com.example.uber_cliente_proyecto.Providers.ConfigProvider
import com.example.uber_cliente_proyecto.Providers.GeoProvider
import com.example.uber_cliente_proyecto.databinding.ActivityInformacionViajeBinding
...
class InformacionViaje : AppCompatActivity(), OnMapReadyCallback, Listener, DirectionUtil.DirectionCallBack,
    SensorEventListener {
    private lateinit var binding: ActivityInformacionViajeBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityInformacionViajeBinding.inflate(layoutInflater)
    ...
    private fun getPrices(distance: Double, time: Double) {
        ...
        configProvider.getPrices().addOnSuccessListener { document ->
            if (document.exists()) {
                ...
                total = if (total < 5.0) prices?.minValor!! else total
                Log.d("PRICES", "new total: $total")

                var minTotal = total - prices?.diferencia!! // TOTAL - 2USD
                Log.d("PRICES", "minTotal: $minTotal")

                var maxTotal = total + prices?.diferencia!! // TOTAL + 2USD
