...
import com.example.uber_cliente_proyecto.Fragmentos.ModalBottomSheetTripInfo
...
import com.example.uber_cliente_proyecto.Utils.CarMoveAnim

import com.example.uber_cliente_proyecto.databinding.ActivityMapaViajeBinding
...
class MapaViaje : AppCompatActivity(), OnMapReadyCallback, Listener, DirectionUtil.DirectionCallBack {
    ...
    private fun getBooking() {
        listenerBooking = bookingProvider.getBooking().addSnapshotListener { document, e ->
            ...
            booking = document?.toObject(Reserva::class.java)



            if (!isBookingLoaded) {
                isBookingLoaded = true
                originLatLng = LatLng(booking?.origenLat!!, booking?.origenLng!!)
                destinationLatLng = LatLng(booking?.destinoLat!!, booking?.destinoLng!!)
            ...
            else if (booking?.estado == "started") {
                startTrip()
            }
            else if (booking?.estado == "finished") {
                finishTrip()
            }
    ...
    private fun finishTrip() {
        ...
        val i = Intent(this, Calificacion::class.java)
