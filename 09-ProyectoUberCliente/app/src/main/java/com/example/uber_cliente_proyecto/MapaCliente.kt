...
import com.example.uber_cliente_proyecto.Fragmentos.ModalBottomSheetMenu
import com.example.uber_cliente_proyecto.Modelos.Cliente
import com.example.uber_cliente_proyecto.Modelos.DriverLocation
...
import com.example.uber_cliente_proyecto.databinding.ActivityMapaClienteBinding
...
class MapaCliente : AppCompatActivity(), OnMapReadyCallback, Listener {
    ...
    private fun removeBooking() {
        ...
        bookingProvider.getBooking().get().addOnSuccessListener { document ->
            ...
            if (document.exists()) {
                val booking = document.toObject(Reserva::class.java)
                if (booking?.estado== "create" || booking?.estado == "cancel") {
...
//                            marker.position = LatLng(location.latitude, location.longitude)
    ...
    private fun instanceAutocompleteOrigin() {
        ...
        autocompleteOrigin?.setCountry("EC")
    ...
    private fun instanceAutocompleteDestination() {
        ...
        autocompleteDestination?.setCountry("EC")
